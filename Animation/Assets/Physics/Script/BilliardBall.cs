...
public class BilliardBall : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        rigidBody.AddForce
            (
            direction * speed,  ForceMode.Acceleration  // 연속적인 힘 (무게적용)
            );
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Pillar"))
        {
            ...
            (
                transform.position.normalized, collision.contacts[0].normal
                );
    ...
    private void OnCollisionStay(Collision collision)
    {
        if (collision.gameObject.CompareTag("Billiard Ball"))
        {
            rigidBody.AddTorque
                (
                    Vector3.up * speed, ForceMode.Impulse // 순간적인 힘 (무게 적용)
                );
